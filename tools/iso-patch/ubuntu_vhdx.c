...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "echo ubuntu > /etc/hostname && echo \"OK: /etc/hostname\" || echo \"FAIL\"\n"
        "command -v hostnamectl >/dev/null 2>&1 && hostnamectl set-hostname ubuntu || echo \"SKIP hostnamectl\"\n"
        "if [ -f /etc/hosts ]; then\n"
        "    sed -i 's/^127\\.0\\.1\\.1.*/127.0.1.1\\tubuntu/' /etc/hosts || true\n"
        "    grep -q '^127.0.1.1' /etc/hosts || printf '127.0.1.1\\tubuntu\\n' >> /etc/hosts\n"
        "fi\n"
        "\n"
        "# --- STEP 3: Timezone (US Eastern) ---\n"
        "echo \"==== STEP 3: timezone ====\"\n"
        "ln -sf /usr/share/zoneinfo/America/New_York /etc/localtime && echo \"OK: localtime\" || echo \"FAIL\"\n"
        "echo America/New_York > /etc/timezone || true\n"
        "\n"
        "# --- STEP 4: Locale + keyboard ---\n"
        "echo \"==== STEP 4: locale + keyboard ====\"\n"
        "echo 'LANG=en_US.UTF-8' > /etc/default/locale || true\n"
        "command -v locale-gen >/dev/null 2>&1 && locale-gen en_US.UTF-8 >/dev/null 2>&1 || echo \"SKIP locale-gen\"\n"
        ...
        "EOF\n"
        "\n"
        "# --- STEP 5: User account: test/test123 ---\n"
        "echo \"==== STEP 5: user account 'test' ====\"\n"
        "if ! id test >/dev/null 2>&1; then\n"
        "    if command -v useradd >/dev/null 2>&1; then\n"
        "        useradd -m -s /bin/bash -c 'Test User' -G sudo,plugdev,lpadmin test \\\n"
        "          && echo \"OK: useradd\" || echo \"FAIL: useradd rc=$?\"\n"
        "        echo 'test:test123' | chpasswd && echo \"OK: chpasswd\" || echo \"FAIL: chpasswd rc=$?\"\n"
        "    else\n"
        ...
        "if id test >/dev/null 2>&1; then\n"
        "    install -d -o test -g test /home/test/.config 2>/dev/null || true\n"
        "    su test -c 'touch /home/test/.config/gnome-initial-setup-done' 2>/dev/null || true\n"
        "fi\n"
        ...
        "\n"
        "# --- STEP 8: install agent ELFs to /usr/local/bin/ ---\n"
        "echo \"==== STEP 8: install agent binaries ====\"\n"
        "if [ -d \"$EXTRAS\" ]; then\n"
        ...
        "              && echo \"OK: install $bin\" || echo \"FAIL: install $bin\"\n"
        "        else\n"
        "            echo \"SKIP $bin (not in extras)\"\n"
        "        fi\n"
        "    done\n"
        "else\n"
        "    echo \"SKIP STEP 8: $EXTRAS not present (no manifest staging)\"\n"
        "fi\n"
        "\n"
        ...
        "        [ -f \"$ko\" ] || continue\n"
        "        install -m 0644 \"$ko\" \"$MOD_DST/\" \\\n"
        "          && echo \"OK: $(basename $ko) -> $MOD_DST/\" \\\n"
        "          || echo \"FAIL: install $ko\"\n"
        "    done\n"
        "    depmod -a \"$TGT_KVER\" && echo \"OK: depmod\" || echo \"FAIL: depmod rc=$?\"\n"
        "else\n"
