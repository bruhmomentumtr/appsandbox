...
/* Allocate the next free host IP in the dynamically-picked NAT subnet
   (see hcn_nat_subnet_prefix). Scans g_vms[] for IPs already in use;
   returns FALSE if the /24 pool is exhausted. */
static BOOL allocate_nat_ip(VmInstance *vm)
{
    BOOL used[256] = { 0 };
    const char *base = hcn_nat_ip_base();   /* e.g. "192.168.42." */
    ...
    for (i = 0; i < g_vm_count; i++) {
        ...
        /* Only consider IPs that share our current subnet base; entries
           from a prior run with a different subnet are stale and ignored
           (their endpoints died with the previous NAT network). */
        if (_strnicmp(g_vms[i].nat_ip, base, base_len) != 0) continue;
        {
            int d = atoi(g_vms[i].nat_ip + base_len);
            if (d >= 2 && d <= 254) used[d] = TRUE;
        }
    }
